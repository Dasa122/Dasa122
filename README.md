<!-- ════════════════════════════════════════════════════════════════ -->
<!--                        Dasa122 · profile                         -->
<!-- ════════════════════════════════════════════════════════════════ -->

<div align="center">

<a href="https://github.com/Dasa122">
  <img src="https://readme-typing-svg.demolab.com?font=JetBrains+Mono&weight=600&size=24&duration=3200&pause=900&color=39FF14&center=true&vCenter=true&width=680&lines=Dasa122%40github%3A~%24+whoami;K%C5%91v%C3%A1g%C3%B3+Gerg%C5%91+%2F%2F+student+developer;Building+the+Filc+school+platform;kotlin+%C2%B7+python+%C2%B7+typescript+%C2%B7+c%2B%2B;%24+systemctl+status+coffee" alt="Dasa122" />
</a>

<br/>

<img src="https://komarev.com/ghpvc/?username=Dasa122&label=visitors&color=39FF14&style=flat-square" alt="profile views" />

</div>

---

```console
Dasa122@github:~$ whoami
Kővágó Gergő  ·  student developer  ·  Budapest, Hungary

Dasa122@github:~$ cat ~/.profile
> builds the Filc school platform  (chronos · iris · mergen)
> self-hosting, automation & robotics on the side
> ships to Android, iOS and the browser

Dasa122@github:~$ ls ~/stack
kotlin/  ktor/  compose-multiplatform/  python/  typescript/
react/   node/  ansible/  proxmox/  docker/  linux/  c++/

Dasa122@github:~$ uptime
up since 2023 — always compiling something _
```

---

### `~/stack`

<p align="left">
  <img src="https://img.shields.io/badge/Kotlin-0D1117?style=flat-square&logo=kotlin&logoColor=39FF14" alt="Kotlin" />
  <img src="https://img.shields.io/badge/Compose%20Multiplatform-0D1117?style=flat-square&logo=jetpackcompose&logoColor=39FF14" alt="Compose Multiplatform" />
  <img src="https://img.shields.io/badge/Ktor-0D1117?style=flat-square&logo=ktor&logoColor=39FF14" alt="Ktor" />
  <img src="https://img.shields.io/badge/Python-0D1117?style=flat-square&logo=python&logoColor=39FF14" alt="Python" />
  <img src="https://img.shields.io/badge/TypeScript-0D1117?style=flat-square&logo=typescript&logoColor=39FF14" alt="TypeScript" />
  <img src="https://img.shields.io/badge/React-0D1117?style=flat-square&logo=react&logoColor=39FF14" alt="React" />
  <img src="https://img.shields.io/badge/Node.js-0D1117?style=flat-square&logo=nodedotjs&logoColor=39FF14" alt="Node.js" />
  <img src="https://img.shields.io/badge/Ansible-0D1117?style=flat-square&logo=ansible&logoColor=39FF14" alt="Ansible" />
  <img src="https://img.shields.io/badge/Proxmox-0D1117?style=flat-square&logo=proxmox&logoColor=39FF14" alt="Proxmox" />
  <img src="https://img.shields.io/badge/Docker-0D1117?style=flat-square&logo=docker&logoColor=39FF14" alt="Docker" />
  <img src="https://img.shields.io/badge/Linux-0D1117?style=flat-square&logo=linux&logoColor=39FF14" alt="Linux" />
  <img src="https://img.shields.io/badge/C%2B%2B-0D1117?style=flat-square&logo=cplusplus&logoColor=39FF14" alt="C++" />
  <img src="https://img.shields.io/badge/Home%20Assistant-0D1117?style=flat-square&logo=homeassistant&logoColor=39FF14" alt="Home Assistant" />
</p>

---

### `~/projects`

| | project | stack | what it is |
|:-:|:--|:--|:--|
| ▸ | [`filcdev/filc`](https://github.com/filcdev/filc) | `TypeScript` | The core Filc stack — the `chronos` API and `iris` web app: timetables, doorlock access control, news and notifications for a real school |
| ▸ | [`filcdev/mergen`](https://github.com/filcdev/mergen) | `Kotlin` · `KMP` | Compose Multiplatform client for **Android & iOS** — the student app |
| ▸ | [`Dasa122/filc-ha`](https://github.com/Dasa122/filc-ha) | `Python` | Home Assistant integration — Filc timetable calendar plus current/next lesson sensors with live countdowns |
| ▸ | [`filcdev/Navigator`](https://github.com/filcdev/Navigator) | `TypeScript` | Indoor-map kiosk that guides people around the school |

```console
Dasa122@github:~$ git remote -v
origin  https://github.com/filcdev/filc.git      # core stack
origin  https://github.com/filcdev/mergen.git    # KMP client
origin  https://github.com/Dasa122/filc-ha.git   # HA integration
```

---

### `~/stats`

<div align="center">

<img height="165" src="https://github-readme-stats.vercel.app/api?username=Dasa122&show_icons=true&hide_border=true&bg_color=0D1117&title_color=39FF14&text_color=C9D1D9&icon_color=39FF14&border_color=30363D" alt="GitHub stats" />
<img height="165" src="https://github-readme-stats.vercel.app/api/top-langs/?username=Dasa122&layout=compact&langs_count=8&hide_border=true&bg_color=0D1117&title_color=39FF14&text_color=C9D1D9&border_color=30363D" alt="Top languages" />

<br/>

<img src="https://streak-stats.demolab.com?user=Dasa122&hide_border=true&background=0D1117&border=30363D&stroke=39FF14&ring=39FF14&fire=39FF14&currStreakLabel=39FF14&sideLabels=39FF14&dates=C9D1D9&currStreakNum=C9D1D9&sideNums=C9D1D9&dayLabels=39FF14" alt="Streak stats" />

<br/>

<img width="100%" src="https://github-readme-activity-graph.vercel.app/graph?username=Dasa122&bg_color=0D1117&color=39FF14&line=39FF14&point=FFFFFF&area=true&hide_border=true&custom_title=Dasa122%20%3A%20commit%20log" alt="Activity graph" />

</div>

---

### `~/contact`

```console
Dasa122@github:~$ echo "let's build something cool"
```

<p align="left">
  <a href="https://github.com/Dasa122"><img src="https://img.shields.io/badge/GitHub-Dasa122-0D1117?style=flat-square&logo=github&logoColor=39FF14" alt="GitHub" /></a>
</p>

<div align="center"><sub><code>Dasa122@github:~$ exit</code></sub></div>
